...
class TestUpdateStatistics(unittest.TestCase):
    def test_update_statistics(self):
        repo_stats = [RepoStats("repo1", 5, 2, 1), RepoStats("repo2", 3, 1, 1)]
        with patch("main.gather_repo_stats", new_callable=AsyncMock, return_value=repo_stats), \
             patch.object(discord_bot_instance, "update_channel_name", new_callable=AsyncMock) as mock_update, \
        ...
        self.assertEqual(embed.title, "📊 Statistics for repo1")

    def test_update_statistics_content(self):
        repo_stats = [
